...
func (h *Handler) refreshGeminiOAuthAccessToken(ctx context.Context, auth *coreauth.Auth) (string, error) {
	...
	conf := &oauth2.Config{
		ClientID:     geminiauth.GetClientID(),
		ClientSecret: geminiauth.GetClientSecret(),
...
func (h *Handler) refreshAntigravityOAuthAccessToken(ctx context.Context, auth *coreauth.Auth) (string, error) {
	...
	form := url.Values{}
	form.Set("client_id", antigravityauth.GetClientID())
	form.Set("client_secret", antigravityauth.GetClientSecret())
